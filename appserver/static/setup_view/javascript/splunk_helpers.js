...
async function update_configuration_file(
  ...
  var options_length = setup_options.length;
  for (var i = 0; i < options_length; i++){
    update_stanzas(
      configuration_file_accessor,
      setup_options[i].stanza,
      setup_options[i].properties
    )
  }
};
async function update_stanzas(
  configuration_file_accessor,
  stanza_name,
  properties
){
...
async function get_current_conf_to_view(
  splunk_js_sdk_service,
  configuration_file_name,
  stanza_name
) {
  ...
  var stanzaProperties = configuration_stanza_accessor.properties();
